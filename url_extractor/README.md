...
```bash
python main.py --url https://example.com --url https://example.org
```
...
1. Structured metadata
   ...
   - `meta[name="author"]`
   - `sameAs` / organization URL fields
2. Link analysis
   - scans all anchor tags
   - prefers LinkedIn company links
   - picks external websites while ignoring social/tracking domains
...
The extractor returns `null` instead of guessing when a signal is weak.

Accepted thresholds:

- `company_name`: strong metadata, heading, content, or corroborated domain inference
- `website`: external link or strong source-domain inference
- `linkedin_url`: explicit company LinkedIn signals only

The final `confidence_score` is capped at `1.0` and is based on the fields that survive the confidence thresholds.
